...
│   │   │   └── [id]/         # DELETE /api/batches/:id
│   │   ├── treatments/       # POST /api/treatments
│   │   └── alerts/           # GET /api/alerts (expiring soon)
│   ├── dashboard/            # Main dashboard page
│   ├── layout.tsx
│   └── page.tsx
├── components/
│   ├── BatchCard.tsx          # Displays a single batch with status badge
│   ├── BatchForm.tsx          # Form to create a new batch
│   └── ExpiryAlert.tsx        # Alert panel for batches expiring within 48h
└── lib/
    ├── db.ts                  # Prisma client singleton
    ├── stability.ts           # Expiry calculation and status logic
...
| `ingredients` | JSON | Array of `{ name, concentration? }` |
...
- **DEGRADING** — 1–48 hours remaining
- **EXPIRED** — past expiry
...
```json
{
  ...
  "phLevel": 3.5,
  "mixedAt": "2026-05-13T09:00:00",
  "ingredients": [
    { "name": "Vitamin C", "concentration": 15 }
  ]
}
```
